...
function removeFirstOccurrences(str, value) {
  ...
  const newStr = str.slice(0, start) + str.slice(end);
...
function countVowels(str) {
  const count = str.match(/[aeiouyAEIOUY]/g);
...
function isPalindrome(str) {
  const normalized = str.toLocaleLowerCase().replace(/[^\w]/g, '');
  ...
  return str
    ...
    .map((char) => {
      if (upperCase.includes(char)) {
        ...
        return upperCase[index < 13 ? index + 13 : index - 13];
      }
      if (loverCase.includes(char)) {
        const index = loverCase.indexOf(char);
        return loverCase[index < 13 ? index + 13 : index - 13];
      }
      return char;
    })
    .join('');
